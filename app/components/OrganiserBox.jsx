...
export const OrganiserBox = ({ organiserBox }) => {
  const title = organiserBox?.title?.value;

  const products = [
    organiserBox?.product1?.reference,
    organiserBox?.product2?.reference,
    organiserBox?.product3?.reference,
    organiserBox?.product4?.reference,
  ].filter(Boolean);
      ...
      <div className="pt-6 px-1.5 grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3 md:gap-4 lg:container m-auto">
        {products.map((product) => (
          <div key={product.id} className="product_warpper">
            <Link to={`/products/${product.handle}`} className="product_images">
              <img
